...
GPROC_EXPORT bool write_PPM_binary(const char *filename, const PixelBuffer &pb)
{
	FILE * fp = fopen(filename, "wb");
	...
	// write the individual pixel values in binary form
	//PixRGBA * pixelPtr = pb.data;


	for (size_t row = 0; row < pb.height; row++) {
		for (size_t col = 0; col < pb.width; col++){

			//fwrite(pixelPtr[col].data, 3, 1, fp);
			int idx = row *pb.width+col;
			fwrite(pb.data[idx].data, 3, 1, fp);
		}
		//pixelPtr = pixelPtr + pb.pixelpitch;
	}
